...
impl SystemKeys {
    fn new() -> Self {
        ...
        let device_key = match exists_key_file("Device", &id_hash) {
            true => {
                let mut device_key =
                    read_key_or_generate_key("Device", &id_hash, "None", false, true);
            ...
            false => read_key_or_generate_key("Device", &id_hash, "None", true, true),
        };
        debug!("Loaded device key");
        let mut system_key = match exists_key_file("System", &id_hash) {
            true => {
                let mut system_key =
                    read_key_or_generate_key("System", &id_hash, "None", false, true);
            ...
            false => read_key_or_generate_key("System", &id_hash, "None", true, true),
        };
...
pub(crate) fn init_user_crypt_secret(
    crypt_secrets: &mut HashMap<String, String>,
    claim: &IdClaim,
    phrase: &str,
) {
...
pub(crate) fn load_token_of_user_certificates(
    sys_did: &str,
    certificates: &mut HashMap<String, String>,
) {
    ...
    let token_raw_data = match token_file.exists() {
        true => match fs::read(token_file) {
            Ok(data) => data,
            Err(e) => {
                println!(
                    "{} [SimpBase] read user_certificates file error: {}",
                    now_string(),
                    e
                );
                return;
            ...
            if let Value::String(secrets_str) = value {
                ...
                if parts.len() >= 4 {
                    ...
                    if verify_signature(&text, sig_base64, &claim.get_cert_verify_key()) {
                        ...
                        debug!(
                            "Valid signature for user certificate at loading: did={}, key={}",
                            claim.gen_did(),
                            key
                        );
                    } else {
...
pub(crate) fn save_user_certificates_to_file(
    sys_did: &str,
    ...
    fs::write(system_token_file.clone(), token_raw_data).expect(&format!(
        "Unable to write file: {}",
        system_token_file.display()
    ))
}
...
pub(crate) fn convert_to_short_user_cert_from_slim(cert_bytes: &[u8]) -> String {
    if cert_bytes.len() == 132 {
        ...
        let timestamp =
            u64::from_le_bytes(cert_bytes[60..68].try_into().unwrap_or(0u64.to_le_bytes()));
...
pub(crate) fn load_token_of_issued_certs(
    sys_did: &str,
    issued_certs: &mut HashMap<String, String>,
) {
    ...
    let token_raw_data = match token_file.exists() {
        true => match fs::read(token_file) {
            ...
            Err(e) => {
                println!(
                    "{} [SimpBase] read user issued certificates file error: {}",
                    now_string(),
                    e
                );
                return;
            ...
            if let Value::String(secrets_str) = value {
                ...
                if parts.len() >= 4 {
                    ...
                    if verify_signature(&text, sig_base64, &claim.get_cert_verify_key()) {
                        ...
                        debug!(
                            "Valid signature for issuer certificate at loading: {}, {}, {}",
                            text,
                            sig_base64,
                            claim.gen_did()
                        );
                        ...
                        debug!(
                            "Invalid signature for issuer certificate at loading: {}, {}, {}",
                            text,
                            sig_base64,
                            claim.gen_did()
                        );
...
pub(crate) fn load_token_by_authorized2system(
    sys_did: &str,
    crypt_secrets: &mut HashMap<String, String>,
) -> String {
    ...
    let admin_did = match token_file.exists() {
        true => {
            let token_raw_data = match fs::read(token_file) {
                ...
                Err(e) => {
                    println!(
                        "{} [SimpBase] read authorized2system file error: {}",
                        now_string(),
                        e
                    );
                    return String::from("");
...
pub(crate) fn save_secret_to_system_token_file(
    ...
    let system_token_file =
        get_path_in_sys_key_dir(&format!("authorized2system_{}.token", sys_did));
    let crypt_key = get_token_crypt_key();
    let token_raw_data = encrypt(json_string.as_bytes(), &crypt_key, 0);
...
pub(crate) fn get_or_create_user_context_token(
    did: &str,
    sys_did: &str,
    nickname: &str,
    id_type: &str,
    ...
    if context.is_default() {
        ...
        let default_private_paths = serde_json::to_string(&vec![
            "config",
            "presets",
            "wildcards",
            "styles",
        ...
        .unwrap_or("".to_string());
        let mut context_default = UserContext::new(
            did,
            sys_did,
            nickname,
            &default_permissions,
        ...
        context_default
            .set_auth_sk_with_secret(&URL_SAFE_NO_PAD.encode(secret_key), default_expire);
        let crypt_key = get_specific_secret_key("context", id_type, symbol_hash, phrase);
        let aes_key_encrypted =
            URL_SAFE_NO_PAD.encode(encrypt(&context_default.get_crypt_key(), &crypt_key, 0));
...
pub(crate) fn get_user_token_from_file(did: &str, sys_did: &str) -> UserContext {
    ...
    match user_token_file.exists() {
        true => {
            let device_key = calc_sha256(&read_key_or_generate_key(
                "Device", &[0u8; 32], "None", false, false,
            ));
            let token_raw_data = match user_token_file.exists() {
                true => match fs::read(user_token_file) {
                    Ok(data) => data,
                    Err(e) => {
                        println!(
                            "{} [SimpBase] read user issued certificates file error: {}",
...
pub(crate) fn update_user_token_to_file(context: &UserContext, method: &str) -> String {
    ...
    match user_token_file.exists() {
        true => {
            ...
            let token_data = decrypt(&token_raw_data, &device_key, 0);
            let mut user_tokens: serde_json::Value =
                serde_json::from_slice(&token_data).unwrap_or(serde_json::json!({}));
            debug!(
                "load user token from file({}): {}",
                user_token_file.display(),
                user_tokens
            );
            if method == "add" {
                let context_string = context.to_json_string();
                let context_raw_data =
                    URL_SAFE_NO_PAD.encode(encrypt(context_string.as_bytes(), &sys_key, 0));
            ...
            fs::write(user_token_file.clone(), token_raw_data).expect(&format!(
                "Unable to write file: {}",
                user_token_file.display()
            ));
            "Ok".to_string()
...
pub(crate) fn get_key_hash_id_and_phrase(
    ...
    match key_type {
        "Device" => _get_key_hash_id_and_phrase(&device_symbol_hash.to_vec(), 0),
        "System" => _get_key_hash_id_and_phrase(&system_symbol_hash.to_vec(), 0),
        _ => {
...
pub(crate) fn get_specific_secret_key(
    key_name: &str,
    key_type: &str,
    symbol_hash: &[u8; 32],
    phrase: &str,
    ...
    debug!(
        "Get specific secret key: Name({}), Type({}), symbol_hash({}), phrase({})",
        key_name,
        key_type,
    ...
    let secret_key =
        StaticSecret::from(derive_key(&com_phrase, &key_name_bytes).unwrap_or([0u8; 32]));
    *secret_key.as_bytes()
}

pub(crate) fn get_random_secret_key(
    key_type: &str,
    symbol_hash: &[u8; 32],
    phrase: &str,
) -> [u8; 32] {
    debug!(
        "Get random secret key: Type({}), symbol_hash({}), phrase({})",
        key_type,
        URL_SAFE_NO_PAD.encode(symbol_hash),
        phrase
    );
...
pub(crate) fn get_signature(
    ...
    get_signature_by_key(
        text,
        &read_key_or_generate_key(key_type, symbol_hash, phrase, false, false),
    )
}
...
pub(crate) fn hkdf_key_deadline(key: &[u8], period: u64) -> [u8; 32] {
    ...
    let input = format!(
        "period:{}",
        if period == 0 { 0 } else { timestamp / period }
    );
...
pub(crate) fn get_user_copy_hash_id_by_source(
    nickname: &str,
    telephone: &str,
    phrase: &str,
) -> String {
    get_user_copy_hash_id(
        nickname,
        URL_SAFE_NO_PAD
            .encode(calc_sha256(telephone.as_bytes()))
            .as_str(),
        phrase,
    )
}
pub(crate) fn get_user_copy_hash_id(
    nickname: &str,
    telephone_base64: &str,
    phrase: &str,
) -> String {
...
pub(crate) fn check_entry_point_of_service(entry_point: &str) -> bool {
    ...
    let now_sec = SystemTime::now()
        ...
        .as_secs();
    let salt = calc_sha256(format!("{}:{}", sysinfo.host_name, now_sec / 600000).as_bytes());
    let entry_point_real = derive_key(&service_id, &salt)
        .unwrap_or([0u8; 32])
        .to_base58();
    if entry_point_real != entry_point {
        let salt =
            calc_sha256(format!("{}:{}", sysinfo.host_name, now_sec / 600000 - 1).as_bytes());
        let entry_point_real = derive_key(&service_id, &salt)
            .unwrap_or([0u8; 32])
            .to_base58();
...
pub(crate) fn read_key_or_generate_key(
    ...
    fn _read_key_or_generate_key(
        ...
        let private_key = match file_path.exists() {
            ...
            true => {
                ...
                if priv_key == [0; 32] {
                    let Ok((_, s_doc)) = SecretDocument::read_pem_file(file_path) else {
                        todo!()
                    };
                    //println!("_read_key_or_generate_key, SecretDocument: {}", file_path.display());
                    priv_key = match EncryptedPrivateKeyInfo::try_from(s_doc.as_bytes())
                        .unwrap()
                        .decrypt(&phrase_bytes)
                    {
                        ...
                        Err(_e) => {
                            if regen {
                                debug!(
                                    "[SimpBase] Read private key error and generate new key: {}",
                                    file_path.display()
                                );
    ...
    let device_key = if through && (key_type == "Device" || key_type == "System") {
        _read_key_or_generate_key(
            device_key_file.as_path(),
            device_phrase.as_str(),
            regen,
            through,
        )
    } else {
        SystemKeys::instance().lock().unwrap().get_device_key()
    };
    let system_key = match key_type {
        "System" | "User" => {
            let (sys_hash_id, sys_phrase) = get_key_hash_id_and_phrase("System", symbol_hash);
            let system_key_file =
                get_path_in_sys_key_dir(&format!(".token_system_{}.pem", sys_hash_id));
            ...
            if through && key_type == "System" {
                _read_key_or_generate_key(
                    system_key_file.as_path(),
                    phrase_text.as_str(),
                    regen,
                    through,
                )
            } else {
                SystemKeys::instance().lock().unwrap().get_system_key()
            }
        }
        _ => device_key,
    };
    match key_type {
        ...
        "User" => {
            ...
            let user_key_file =
                get_path_in_sys_key_dir(&format!(".token_user_{}.pem", user_hash_id));
            let phrase_text = format!(
                "{}|{}|{}",
                URL_SAFE_NO_PAD.encode(device_key.as_slice()),
                phrase,
...
fn generate_new_key_and_save_pem(file_path: &Path, phrase: &[u8; 32]) -> [u8; 32] {
    ...
    if let Some(file_name) = file_path.file_name() {
        ...
        if file_name_str.contains("device") {
            let seed = derive_key(
                &calc_sha256(format!("{}{}", sysinfo.disk_uuid, sysinfo.os_time).as_bytes()),
            ...
            let seed = derive_key(
                &calc_sha256(format!("{}{}", sysinfo.root_dir, sysinfo.root_time).as_bytes()),
...
pub(crate) fn exists_key_file(key_type: &str, symbol_hash: &[u8; 32]) -> bool {
    ...
    let key_file = get_path_in_sys_key_dir(&format!(
        ".token_{}_{}.pem",
                ...
                if path.is_file() {
                    if let Some(file_name) = path.file_name() {
                        if let Some(file_name_str) = file_name.to_str() {
                            let contains_filter =
                                filters.iter().any(|filter| file_name_str.contains(filter));
                            let ends_with_suffix = suffixes
                                .iter()
...
pub(crate) fn export_identity(
    ...
    debug!(
        "export, encrypted_identity: len={}, {}",
        encrypted_identity.len(),
        URL_SAFE_NO_PAD.encode(encrypted_identity.clone())
    );
    encrypted_identity
}
...
pub(crate) fn import_identity(
    ...
    let identity = &encrypted_identity[2..];
    if *vcode == calc_sha256(identity)[..2] {
        let telephone_number = u64::from_le_bytes(encrypted_identity[2..10].try_into().unwrap());
        let telephone = if telephone_number == 0 {
            String::new()
        } else {
        ...
        let secret_key = derive_key(
            phrase.as_bytes(),
            &URL_SAFE_NO_PAD.decode(symbol_hash_base64).unwrap(),
        )
        .unwrap();
        let identity_secret = decrypt(encrypted_secret, &secret_key, 0);
        debug!(
        ...
        if identity_secret.len() > "Unknown".len() {
            ...
            let telephone_source = if telephone.is_empty() {
                "-"
            } else {
                &telephone
            };
            let telephone_hash =
                calc_sha256(format!("{}:telephone:{}", nickname, telephone_source).as_bytes());
            let telephone_base64 = URL_SAFE_NO_PAD.encode(telephone_hash);
            let id_card_base64 = URL_SAFE_NO_PAD.encode(calc_sha256(
                format!("{}:id_card:{}", nickname, "-").as_bytes(),
            ));
            let symbol_hash = calc_sha256(
            ...
            let mut user_claim = LocalClaims::generate_did_claim(
                "User",
                nickname,
                telephone_for_claim,
                None,
            ...
            println!(
                "{} [SimpBase] import_identity: Invalid identity secret",
                now_string()
            );
            IdClaim::default()
        }
        ...
        println!(
            "{} [SimpBase] import_identity: Invalid identity string",
            now_string()
        );
        IdClaim::default()
    }
...
pub(crate) fn import_identity_qrcode(
    encrypted_identity: &Vec<u8>,
    ...
    debug!(
        "import_identity_qrcode: did={} cert={}, encrypted_identity: len={}, {}",
        user_did,
        user_cert,
        encrypted_identity.len(),
        URL_SAFE_NO_PAD.encode(encrypted_identity)
    );
    let vcode = &encrypted_identity[..2];
    if *vcode == calc_sha256(&encrypted_identity[2..])[..2] {
        let telephone =
            u64::from_le_bytes(encrypted_identity[2..10].try_into().unwrap_or([0u8; 8]));
        let nickname = std::str::from_utf8(&encrypted_identity[78..])
            .unwrap_or("")
            .to_string();
        let telephone_for_claim = if telephone == 0 {
            None
        } else {
            Some(telephone.to_string())
        };
        ...
        let identity_file =
            get_path_in_sys_key_dir(&format!("user_identity_{}.token", user_hash_id));
        fs::write(
            identity_file.clone(),
            URL_SAFE_NO_PAD.encode(encrypted_identity),
        )
        ...
        debug!(
            "{} [SimpBase] Import from qrcode and save identity_file: did={}, nickname={}",
            now_string(),
            user_did,
            nickname
        );
        if telephone == 0 {
            ...
            (user_did, nickname, telephone.to_string(), user_cert)
        }
    } else {
...
pub(crate) fn decrypt_issue_cert_with_vcode(vcode: &str, issue_cert: &str) -> String {
    let result_certificate = URL_SAFE_NO_PAD
        .decode(issue_cert)
...
pub(crate) fn decrypt_text_with_vcode(vcode: &str, encrypted_text: &str) -> String {
    let encrypted_text_bytes = URL_SAFE_NO_PAD
