...
use crate::dids::claims::{GlobalClaims, IdClaim, LocalClaims};
...
pub(crate) static TOKIO_RUNTIME: Lazy<Runtime> =
...
impl DidToken {
    ...
    pub fn new() -> Self {
        ...
        let sysinfo_handle = TOKIO_RUNTIME.spawn(async move { SystemInfo::generate().await });
        ...
        let mut guest_key = match token_utils::exists_key_file("User", &guest_symbol_hash) {
            true => {
                let mut guest_key = token_utils::read_key_or_generate_key(
                    "User",
                    &guest_symbol_hash,
                    &guest_phrase,
                    false,
                ...
                if guest_key == [0u8; 32] {
                    ...
                    guest_key = token_utils::read_key_or_generate_key(
                        "User",
                        &guest_symbol_hash,
                        &guest_phrase,
                        true,
            ...
            false => token_utils::read_key_or_generate_key(
                "User",
                &guest_symbol_hash,
                &guest_phrase,
                true,
        ...
        let (guest_hash_id, guest_phrase) =
            token_utils::get_key_hash_id_and_phrase("User", &guest_symbol_hash);
        ...
        let sysinfo =
            TOKIO_RUNTIME.block_on(async { sysinfo_handle.await.expect("Sysinfo Task panicked") });
        ...
        debug!(
            "DidToken context build finished: {} -> crypt_secrets.len={}",
            crypt_secrets_len,
            crypt_secrets.len()
        );
    ...
    pub fn sign_and_issue_cert_by_system(
        &mut self,
        item: &str,
        for_did: &str,
        for_sys_did: &str,
    ...
    pub fn sign_and_issue_cert_by_did(
        &mut self,
        issuer_did: &str,
        item: &str,
        for_did: &str,
        ...
        if !issuer_did.is_empty()
            && !for_did.is_empty()
            && !for_sys_did.is_empty()
            && !item.is_empty()
            && !memo.is_empty()
            && IdClaim::validity(issuer_did)
            && IdClaim::validity(for_did)
            && IdClaim::validity(for_sys_did)
            && item.len() < 32
            && memo.len() < 256
        {
            let unknown = "Unknown".to_string();
            let cert_secret_base64 = self
                .crypt_secrets
                .get(&issue_key!(issuer_did))
                .unwrap_or(&unknown);
            if cert_secret_base64 != "Unknown" {
                let cert_secret = token_utils::convert_base64_to_key(cert_secret_base64);
                if cert_secret != [0u8; 32] {
                    let item_key = token_utils::derive_key(
                        item.as_bytes(),
                        &token_utils::calc_sha256(&cert_secret),
                    )
                    .unwrap_or([0u8; 32]);
                    if item_key != [0u8; 32] {
                        ...
                        let sig =
                            URL_SAFE_NO_PAD.encode(self.sign_by_issuer_key(
                                &cert_text,
                        ...
                        if for_sys_did == self.did {
                            return (
                                format!("{}|{}|{}", issuer_did, for_did, item),
                                format!("{}|{}", cert_text, sig),
                            );
                        } else {
                            return (
                                format!("{}|{}|{}", issuer_did, for_did, item),
                                self.encrypt_for_did(
                                    format!("{}|{}", cert_text, sig).as_bytes(),
                                    for_sys_did,
                                    0,
                                ),
                            );
        ...
        println!(
            "{} [SimpBase] Sign and issue a cert by did: invalid params",
    ...
    pub fn decrypt_by_did(&mut self, ctext: &str, by_did: &str, period: u64) -> String {
        let text =
            self.decrypt_bytes_by_did(&URL_SAFE_NO_PAD.decode(ctext).unwrap(), by_did, period);
    ...
    pub fn encrypt_bytes_for_did(&mut self, text: &[u8], for_did: &str, period: u64) -> Vec<u8> {
        let self_crypt_secret = token_utils::convert_base64_to_key(
            self.crypt_secrets.get(&exchange_key!(self.did)).unwrap(),
        );
    ...
    pub(crate) fn add_crypt_secret_for_user(
        &mut self,
        user_claim: &IdClaim,
        phrase: &str,
    ) -> String {
        ...
        if self.crypt_secrets.len() > crypt_secrets_len {
            token_utils::save_secret_to_system_token_file(
                &mut self.crypt_secrets,
                &self.did,
                &self.admin,
            );
        }
    ...
    pub(crate) fn remove_crypt_secret_for_user(&mut self, user_did: &str) {
        ...
        if exchange_key_value.is_some() || issue_key_value.is_some() {
            let _ = token_utils::save_secret_to_system_token_file(
                &mut self.crypt_secrets,
                &self.did,
                &self.admin,
            );
    ...
    pub fn get_local_crypt_text(&mut self, ua_hash: &str) -> [u8; 32] {
        token_utils::calc_sha256(
            format!(
                "{}|{}|{}",
                ua_hash,
                self.crypt_secrets[&exchange_key!(self.did)],
                self.crypt_secrets[&exchange_key!(self.device)]
            )
            .as_bytes(),
        )
    }
    ...
    pub fn get_or_create_register_cert(&mut self, user_did: &str) -> String {
        let register_cert = self
            .certificates
            .lock()
            .unwrap()
            .get_register_cert(user_did);
        ...
        if user_did == self.guest || (node_mode != "online" && user_did == admin_did) {
            let register_cert = self.issue_local_member_cert(user_did);
            println!("sign and issue member cert by system: user_did={}, sys_did={}, node_type={}, cert={}", user_did, &self.did, node_mode, register_cert);
            register_cert
        } else {
            "Unknown".to_string()
        }
    ...
    pub fn issue_local_member_cert(&mut self, user_did: &str) -> String {
        ...
        let (_issue_cert_key, issue_cert) =
            self.sign_and_issue_cert_by_system("Member", user_did, &system_did, "User");
        if issue_cert == "Unknown" {
            return "Unknown".to_string();
        }
    ...
    pub fn is_registered(&mut self, user_did: &str) -> bool {
        ...
        if self.node_mode == "online" {
            let text = format!(
                "{}|{}|{}|{}|{}|{}",
                TOKEN_ENTRYPOINT_DID, user_did, "Member", encrypt_item_key, memo_base64, timestamp
            );
            let claim = LocalClaims::load_claim_from_local(TOKEN_ENTRYPOINT_DID);
            debug!(
                "did({}), cert_str({}), cert_text({}), sign_did({})",
                user_did,
                cert_str,
                text,
                claim.gen_did()
        ...
        if !self.upstream_did.is_empty() && self.node_mode == "online" {
            let text = format!(
                "{}|{}|{}|{}|{}|{}",
                self.upstream_did, user_did, "Member", encrypt_item_key, memo_base64, timestamp
            );
        ...
        debug!(
            "did({}), cert_str({}), sign_did({})",
            user_did,
            cert_str,
            claim.gen_did()
        );
        debug!(
            "text_system:{}, signature_str:{}, cert_verify_key:{}",
            text,
            signature_str,
            URL_SAFE_NO_PAD.encode(claim.get_cert_verify_key())
        );
        if token_utils::verify_signature(&text, &signature_str, &claim.get_cert_verify_key()) {
            return true;
        }
...
pub(crate) fn get_system_vars() -> (String, String, String, String, String, String) {
    ...
    let (dev_hash_id, device_phrase) =
        token_utils::get_key_hash_id_and_phrase("Device", &zeroed_key);
    let (sys_hash_id, system_phrase) =
        token_utils::get_key_hash_id_and_phrase("System", &zeroed_key);
    let guest_symbol_hash = get_key_symbol_hash("Guest");
    let (guest_hash_id, guest_phrase) =
        token_utils::get_key_hash_id_and_phrase("User", &guest_symbol_hash);
...
pub(crate) fn get_system_key_name() -> (String, String, String) {
    ...
    let guest_name = if !sysinfo.disk_uuid.is_empty() {
        ...
        format!("guest_{}", safe_prefix)
            .chars()
            .take(24)
            .collect::<String>()
    } else {
        let random_chars = token_utils::calc_sha256(
            format!(
                "{}",
                SystemTime::now()
                    .duration_since(SystemTime::UNIX_EPOCH)
    ...
    system_name = token_utils::truncate_nickname(&format!(
        "{}_{}",
        system_name,
        &token_utils::calc_sha256(sysinfo.root_dir.as_bytes()).to_base58()[..4]
    ));
...
pub(crate) fn get_key_symbol_hash(key_type: &str) -> [u8; 32] {
    ...
    match key_type {
        ...
        "System" => IdClaim::get_symbol_hash_by_source(
            &system_name,
            None,
            Some(format!("{}:{}", root_dir.clone(), disk_uuid.clone())),
        ),
        "Guest" => IdClaim::get_symbol_hash_by_source(
            &guest_name,
            None,
            Some(format!("{}:{}", root_dir.clone(), disk_uuid.clone())),
        ),
